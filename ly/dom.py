...
# prevent picking old stale node.py from package
...
class Printer(object):
    ...
    def quoteString(self, text):
        if self.typographicalQuotes:
            text = re.sub(r'"(.*?)"', self.primary_quote_left
                          + r'\1' + self.primary_quote_right, text)
            text = re.sub(r"'(.*?)'", self.secondary_quote_left
                          + r'\1' + self.secondary_quote_right, text)
            text = text.replace("'", '\u2018')
...
class Assignment(Container):
    ...
    def ly(self, printer):
        return "{0} = {1}".format(
            self.name, super(Assignment, self).ly(printer))
