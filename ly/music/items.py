...
class Item(ly.node.WeakNode):
    ...
    def end_position(self):
        ...
        def ends():
            ...
            # end pos of Item or Token instances in attributes, such as
            # duration etc
    ...
    def has_output(self, _seen_docs=None):
        ...
        for n in self:
            if isinstance(n, (Music, Markup)):
                ...
                if doc and doc not in _seen_docs and doc.has_output(
                        _seen_docs):
                    return True
    ...
    def time_length(self, start, end):
        ...
        if start_evts:
            ...
            if end_evts and start_evts[0][0] is end_evts[0][0]:
                ...
                for i, ((evt, s), (end_evt, end_s)) in enumerate(
                        zip(start_evts, end_evts)):
                    if evt is end_evt:
...
class UserCommand(Music):
    ...
    def name(self):
        """Return the name of this user command
...
class Language(Item):
    r"""A command (\language or certain \include commands)
    that changes the pitch language.
            ...
            if isinstance(i, Assignment) and i.name() == self.name():
                ...
                for j in i:
                    if isinstance(j, SchemeList):
                        for k in j:
                            if (isinstance(k, SchemeItem)
                                    and k.token == 'define-markup-command'):
                                for l in j[1::]:
                                    if isinstance(l, SchemeList):
                                        for m in l:
                                            if (isinstance(m, SchemeItem)
                                                    and m.token
                                                    == self.name()):
                                                return i
                                            break
...
class String(Item):
    ...
    def plaintext(self):
        """Return the plaintext value of this string,
        without escapes and quotes.
...
class Scheme(Item):
    ...
    def get_pair_ints(self):
        ...
        result = [int(i.token)
                  for i in self.find(SchemeItem) if i.token.isdigit()]
        if len(result) >= 2:
